...
```
```{r}
clean_names(vertebrate)
```

```{r}
variable_classes <- sapply(vertebrate, class)
variable_classes
```
...
```{r}
vertebrate$order <- as.factor(vertebrate$order)
vertebrate$taxon <- as.factor(vertebrate$taxon)

levels(vertebrate$taxon)
levels(vertebrate$order)
```

**4. What taxa are represented in the `homerange` data frame? Make a new data frame `taxa` that is restricted to taxon, common name, class, order, family, genus, species.**  
```{r}
select (vertebrate, "taxon")
```
```{r}
names(vertebrate)
```
...
```{r}
taxa <- select(vertebrate, "taxon", "common.name", "class", "order", "family", "genus", "species")
taxa
```
...
```{r}
table(vertebrate$trophic.guild)
```
