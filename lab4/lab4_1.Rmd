...
```{r}
fish <- readr::read_csv("data/Gaeta_etal_CLC_data.csv") #resdr is...
```
```{r}
help() # insted you can use: ?? fish 
```
...
```{r}
select(fish, "fish_id", "length") #you can also reverse it, or assighn to the object
```
...
```{r}
select(fish, fish_id:length) # : helps to not exclude data in between
```
...
```{r}
select(fish, matches("a.+er")) #handy bit of code for the project (parts of words)
```
...
```{r}
select_if(fish, ~!is.numeric(.)) # (.) all of the cells, ! - all other then mentions data 
```
...
```{r}
mass_new <- select(mammals_new, -order, -"family")
mass_new
```
...
```{r}
mass_1 <- select(mammals_new, contains("mass"))
mass_1
```

8. Select all of the columns that are of class `character`.  
```{r}
mass_2 <- select_if(mammals_new, ~!is.numeric(.))
mass_2
```
...
```{r}
mammals_new_low <- select(mammals_new, tolower)
mammals_new_low
```
```{r}
library(janitor)
...
```{r}
#select_all(mammals_new, ~str_replace(., " ", "_"))
clean_names(mammals)
```
