...
```{r}
p + theme_linedraw()+ #use link above for themes
...
```{r}
ls("package:ggthemes")[grepl("theme_", ls("package:ggthemes"))] #here I can shop for themes 
```
...
```{r}
display.brewer.pal(3,"Blues") #sequential palette, use webiste on the top
```
...
```{r}
q+scale_color_brewer(palette = "GnBu")+ #here use color not fill, because it is a scaterplot 
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle = 60, hjust=1))+
  labs(title = "Observations by Taxon in Homerange Data",
       x = NULL,
...
```{r}
my_palette <- paletteer_d("ggsci::springfield_simpsons") #add package and palete fromm the table 
```
...
homerange %>% 
  ggplot(aes(x=taxon, y=log10.mass))+
  geom_boxplot()+
  facet_grid(trophic.guild~thermoregulation)+ #more then one categorical variables 
  theme(axis.text.x = element_text(angle = 60, hjust=1))
```
