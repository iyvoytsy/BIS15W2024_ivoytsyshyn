...
```
## here() starts at C:/Users/illya/OneDrive/Робочий стіл/BIS15L/BIS15W2024_ivoytsyshyn
```
...
```r
sydneybeaches #this is the long data, we can see it based on the column date.
```
...
```r
sydneybeaches_long_2 <- sydneybeaches_long %>% 
...
```r
sydneybeaches_long_3 <- sydneybeaches_long_2 %>%
  group_by(site, year) %>%
  summarise(mean_enterococci = mean(enterococci_cfu_100ml, na.rm = TRUE)) %>%
...
```r
sydneybeaches_long_3$"2013" <- as.numeric(sydneybeaches_long_3$"2013")
```
...
```r
colnames(sydneybeaches_long_3)
```

```
## [1] "site" "2013" "2014" "2015" "2016" "2017" "2018"
```
