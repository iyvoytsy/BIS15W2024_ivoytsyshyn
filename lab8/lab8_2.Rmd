...
```{r}
life_history <- read_csv("data/mammal_lifehistories_v3.csv", na = c("NA", " ", ".", "-999", "not measured")) %>% clean_names() #important
```
...
```{r}
naniar::miss_var_summary(life_history) #important
```
...
```{r}
vis_miss(life_history, warn_large_data = F)
```
