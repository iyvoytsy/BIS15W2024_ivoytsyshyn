...
penguins %>% 
  group_by(species) %>% 
  summarize(mean_bill_l=mean(bill_length_mm, na.rm = T))
```
...
penguins %>%
  group_by(island, sex) %>%
  summarise(n_missing = sum(is.na(species) | is.na(sex))) #important
```
```{r}
penguins %>%
  count(island, sex) #important
```
...
```{r}
penguins %>%
  summarize(across(contains("mm"), mean, na.rm=T)) #important
```
