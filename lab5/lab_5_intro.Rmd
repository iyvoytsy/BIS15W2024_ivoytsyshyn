...
```{r}
fishlength <- select(fish, "lakeid", "length") #reshape the data frame 
```


3. Filter the `fish` data to include the samples from lake "BO".  
```{r}
fishlength_bo <- filter(fishlength, lakeid == "BO") #looks and pulls out data of interest 
```
...
```{r}
fishlength_bo_mean <- mean(fishlength_bo$length, na.rm = T)
fishlength_bo_mean
```

#tidyverse is a big libary while the dplyr is packge for it.
