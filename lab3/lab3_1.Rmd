...
```{r}
hbrids <- data.frame(sex=Sex, length=Length, weight_g=Weight)
```
...
```{r}
temp <- hsprings[,1] #same as data matrix
temp
mean(temp)
```
...
```{r}
write.csv(hsprings, "hprings.csv", row.names = FALSE)
```
